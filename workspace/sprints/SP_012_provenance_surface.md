...
- **Iteration 3** — post-merge follow-up review (PR #1), correctness. HIGH: (H1) `render_contradictions` emitted a one-sided conflict line when only one side resolved a marker (guard was `refs == ""`, i.e. *neither* side) → fixed to require **both** sides (`if not (a and b): continue`); (H2) `_gather_links` resolved links only via `from_entity_id`, so a link-pair contradiction whose other link hangs off a *different* entity was never put in grounding — the synth saw the conflict on the bundle but couldn't attribute it → fixed to resolve contradiction-side links by link id (scan-all only when a needed side is missing) and never drop a side under the cap (+engine test). MEDIUM: `None` `object_value` rendered literally ("runway: None …") → now "(no stated value)"; the prompt's `cites` JSON example showed only `C1` (biased the model away from now-resolving `S#`/`L#`) → example shows a mixed `["C1","S2","L1"]`. Files reviewed: synthesis.py, engine.py, prompts/ask_synthesis.md, test_synthesis.py, test_engine.py. **Resolution:** all fixed; 377 passed / 36 skipped, mypy clean, gateway green.
