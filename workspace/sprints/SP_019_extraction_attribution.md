...
- **Iteration 2** — paid re-record verification (operator-approved, **DONE 2026-06-11**): 0 CRITICAL/HIGH. Re-recorded the 9 smoke docs with the new prompt (Sonnet+Voyage, no Opus, all empty_extractions=0 / truncated=0) and re-graded with `check_extraction`. **Result: golden recall 8/11 → 11/11 (100%), precision 100%, mismatch=0.** All three re-record-gated facts landed (CRM re-subject + the two flagged "stretch": Confluence clean value, top_contributor emission). Behavioral closure (Rule 21): the metric-as-subject / milestone-attribution symptoms were replayed against the system and the golden bar cleared. Recorded in `workspace/acceptance/SP010_finalmile_run.md`. Files reviewed: re-record [REC] ledger + grader output.
