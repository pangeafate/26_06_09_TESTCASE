...
def test_format_report_flags_ungrounded_evidence_in_sample():
    # evidence text not contained in chunk_text (even normalized) → ungrounded flag
    text = format_report(_report(sample=[_rec(evidence="NOT IN CHUNK", chunk_text="other body")]))
    assert "ungrounded" in text
    assert "non-verbatim" not in text


def test_format_report_flags_non_verbatim_evidence_in_sample():
    # evidence matches the chunk only after case/whitespace folding → non-verbatim, NOT ungrounded
    text = format_report(_report(sample=[_rec(evidence="revenue: sgd 14.2m")]))
    assert "non-verbatim" in text
